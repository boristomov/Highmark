...
export const cartReducer = (state = init, action) => {
  switch (action.type) {
    ...
    case SET_QUANTITY:
      const setProductId = action.product_id.id;
      const setQuantity = Math.max(1, parseInt(action.quantity) || 1);
      const setCart = state.cart.reduce((cartAcc, product) => {
        if (product.id === setProductId) {
          cartAcc.push({
            ...product,
            qty: setQuantity,
          });
